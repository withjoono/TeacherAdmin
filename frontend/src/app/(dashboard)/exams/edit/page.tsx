...
function ExamEditContent() {
  ...
  const examId = parseInt(searchParams.get("id") ?? "0", 10);
  ...
  const handleSave = async (status?: "draft" | "published") => {
    if (!name.trim()) { setError("시험 이름을 입력해주세요."); return; }
    setSaving(true);
    setError(null);
    try {
      await updateTeacherExam(examId, {
        ...
        questions: questions.map((q) => ({
          ...
          answerText: q.questionType !== "objective" ? q.answerText || undefined : undefined,
  ...
  return (
    <PageContainer className="space-y-6">
      ...
      <section className="space-y-4">
        <h2 className="text-sm font-semibold">문항 ({questions.length}개)</h2>
        ...
        <div className="rounded-xl border bg-card p-4">
          ...
          <div className="mt-3 space-y-3">
            ...
            <BulkGroup label="객관식 보기 수">
              {[4, 5].map((c) => (
                <Button key={c} variant="outline" size="sm" onClick={() => setAllChoiceCount(c)}>
                  {c}지선다
                </Button>
              ))}
            </BulkGroup>
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                />
                <Button variant="secondary" onClick={applyBulkAnswers}>일괄 적용</Button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 pb-4">
        <Button onClick={() => handleSave("published")} disabled={saving || missingAnswers > 0}>
...
export default function ExamEditPage() {
  return (
    <Suspense fallback={<PageContainer><Spinner full label="불러오는 중..." /></PageContainer>}>
      <ExamEditContent />
    </Suspense>
  );
}
